...
export default function ModeButton({ mode, title, bgColor, hoverColor }: ModeButtonProps) {
  ...
  return (
    <button
      onClick={handleClick}
      disabled={loading}
      className={`w-40 ${bgColor} ${!loading && hoverColor} text-white font-bold py-4 sm:py-3 px-2 rounded transition-colors flex items-center justify-center min-h-[40px] ${loading ? 'cursor-not-allowed' : ''}`}
    >
      {loading ? (
