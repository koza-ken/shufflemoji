...
export default function ModeButton({ mode, title, bgColor, hoverColor }: ModeButtonProps) {
  ...
  return (
    <button
      onClick={handleClick}
      disabled={loading}
      className={`w-40 ${bgColor} ${!loading && `hover:${hoverColor}`} text-white font-bold py-2 px-4 rounded transition-colors flex items-center justify-center min-h-[40px] ${loading ? 'cursor-not-allowed' : ''}`}
    >
      {loading ? (
